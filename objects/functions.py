...
def draw(scr: pygame.Surface, dct):
    match objects.constants.STATUS:
        ...
        case 'DIED':
            ...
            if dct['counter']:
                dct['counter'] -= 1
                percent = get_percent(dct)
                if percent > dct['percent']:
                    dct['percent'] = percent
                    dct['new_best'].update(percent)
                    scr.blit(dct['new_best'], (180, 300))
            else:
                dct['resetter']()
        case 'WIN':
            scr.blit(dct['level_bg'], (0, 0))
...
def get_percent(dct):
    cell = dct['board'].get_cell((dct['player'].rect.x, dct['player'].rect.y))
    return int(cell[1] / len(dct['board'].board) * 100)
